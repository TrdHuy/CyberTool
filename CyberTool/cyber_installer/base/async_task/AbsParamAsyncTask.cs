...
namespace cyber_installer.@base.async_task
{
    internal abstract class AbsParamAsyncTask : ParamAsyncTask
    {
        private Action<AsyncTaskResult>? _completedCallback;
        private Func<object, bool>? _extraConditionToExecute;
        public AbsParamAsyncTask(object param
            , string name
            ...
            : base(mainFunc, cancellationTokenSource, param, canExecute, callback, name, estimatedTime, delayTime, reportDelay)
        {
            ...
            _extraConditionToExecute = canExecute;
        ...
        private bool _IsTaskPossile(object param)
        {
            return IsTaskPossible(param)
                && (_extraConditionToExecute?.Invoke(param) ?? true);
        }
