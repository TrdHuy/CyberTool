...
namespace log_guard.implement.flow.filter_engines
{
    internal class AdvanceSyntaxFilterEngine : NormalFilterEngine
    {
        ...
        private bool OrOperateCalC(string A, string B, string Cmp)
        {
            ...
            {
                res = 1;
                MatchedWords.Add(new MatchedWordVO(startA, A, Cmp));
                IsMatchLstEmpty = false;
            }
            if (startB != -1)
            {
                res = 2;
                MatchedWords.Add(new MatchedWordVO(startB, B, Cmp));
                IsMatchLstEmpty = false;
            }
